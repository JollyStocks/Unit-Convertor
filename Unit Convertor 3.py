...
unit_convertor.geometry("730x350")
unit_convertor.resizable(False, False)

# Theme
unit_convertor.tk.call("source", "C:/Users/Morri/Downloads/App/Azure/azure.tcl")
...
options = ["Picometer [pm]", "Nanometer [nm]", "Micrometer [μm]", "Millimeters [mm]",
           "Centimeters [cm]", "Meters [m]", "Kilometers [km]"]

selected_category = tk.StringVar(value="Length")
...
from_combo = ttk.Combobox(length_frame, values=options, width=20, state="readonly")
from_combo.set("Select a unit")
from_combo.grid(row=1, column=1)

to_lbl = ttk.Label(length_frame, text="To")
to_lbl.grid(row=2, column=0, sticky="w")

dispbx_output = ttk.Entry(length_frame, width=30)
dispbx_output.grid(row=3, column=0, padx=(0, 10))

to_combo = ttk.Combobox(length_frame, values=options, width=20, state="readonly")
...
ttk.Label(mass_frame, text="To").grid(row=2, column=0, sticky="w")
ttk.Entry(mass_frame, width=30).grid(row=3, column=0, padx=(0, 10))
ttk.Combobox(mass_frame, values=[], width=20, state="readonly").grid(row=3, column=1)

# Radiobuttons
def raise_frame(frame):
    frame.tkraise()

ttk.Radiobutton(unit_convertor, text="Length", variable=selected_category,
                value="Length", command=lambda: raise_frame(length_frame)).grid(
                row=1, column=0, padx=10, pady=5, sticky="w")

ttk.Radiobutton(unit_convertor, text="Mass", variable=selected_category,
